...
def create_order(db: Session, order: OrderSchema) -> dict:
    ...
    for orderItem in order.items:
        ...
        newOrderObj.items.append(
            OrderItemModel(quantity=orderItem.quantity, itemId=orderItem.id, price=orderDetails.price))

    # if no payment amount is provided, use the calculated amount as total
    if order.payment is None:
        newOrderObj.payment = orderTotal
